...
Every year people die from accidents on board ships, and as good as the evacuation plans may be now,                           % Write in the present time
they will need improvement to ensure that as many passengers as possible survives. This project will work                            %"This project"
 on modifying existing algorithms that will find the optimal path of evacuation during a simulated crisis, and which could         % Do we need to further emphasise the simulation?
...
Our main focus will be on the Ant Colony Optimization, a pathfinding algorithm which finds a path through
 the environment to the desired destination by mimicking the behavior of an ant colony. It does so by sending    % -> virtually sending out
out ants that run randomly around until they find the destination and report back on the path they took.             % -> not completely random, biased towards pheromones. Be clearer
...
Every year people die in boating accidents, we hope that our work can be                                                                         % Too similar to first line in introduction
...
We aim to evaluate several algorithms and determine which algorithm will provide us with the         % evaluate AND approach
best route between two points in dynamic graphs with nodes and vertices. Our hypothesis
is that Ant Colony Optimization (ACO) will perform better than conventional pathfinding % the most commonly used
...
Secondly, we hypothesize that ACO will outperform djikstra's pathfinding algorithm       % Djikstra's
...
Finally, we hypothesize that ACO will produce better results than djikstra's pathfinding algorithm                 % How to structure these points.... -> its fine
...
we will not create completely new algorithms, a fully fleshed out system that would distribute the % base ourselves on well known algorithms
...
evacuees before the issue occurs or, failing that, we will calculate the optimal way to solve the problem.   %optimal is not a good word here

%\section{Target audience}

\section{Report outline}

The master thesis will contain 4 chapters: Introduction, Methods, Results and Conclusion.                %Write in the active form, will contain -> contains
